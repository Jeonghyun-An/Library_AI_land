...
- 조항 번호 정확 검색 (exact_article 전략)
- 개념 키워드 확장

v2.2 변경사항:
- multi_article 전략 추가: 복수 조항("제10조와 제11조") 감지
- exact_article 전략: 단일 조항 검색 시에만 적용
- comparative_constitution_router.py에서 직접 호출됨
"""
...
class ConstitutionSearchOptimizer:
    ...
    search_strategy 값:
    - 'exact_article': 단일 조항 번호 검색 (예: "제10조")
    - 'multi_article': 복수 조항 검색 (예: "제10조와 제11조 비교")
    - 'concept': 개념 키워드 기반 검색
    - 'hybrid': 일반 하이브리드 검색 (기본값)
    """
    ...
    def __init__(self):
        ...
        self.concept_keywords = {
            ...
            '언론의 자유': ['언론', '출판', '집회', '결사'],
            '재산권': ['재산', '소유', '재산권'],
            '통치구조': ['대통령', '국회', '법원', '헌법재판소', '정부', '행정부', '입법부', '사법부'],
            '기본권': ['기본권', '인권', '권리', '자유'],
        }
    ...
    def optimize_query(self, query: str, lang: str = "ko") -> Dict[str, Any]:
        ...
        Returns:
            {
                'original_query': str,
                'optimized_query': str,
                'article_filters': List[str],       # 추출된 조항 번호 목록
                'chapter_filters': List[str],
                'paragraph_filters': List[str],
                'concept_keywords': List[str],
                'search_strategy': str,
                    - 'exact_article': 단일 조항 (제N조)
                    - 'multi_article': 복수 조항 (제N조, 제M조)
                    - 'concept': 개념 키워드
                    - 'hybrid': 일반
            }
        """
        ...
        # 조항 번호 추출
        article_matches = []
        ko_pattern = self.article_patterns.get('ko')
        en_pattern = self.article_patterns.get('en') if lang == 'en' else None

        if ko_pattern:
            article_matches += ko_pattern.findall(query)
        if en_pattern:
            article_matches += en_pattern.findall(query)

        if article_matches:
            result['article_filters'] = list(set(article_matches))

        # 장 번호 추출
        chapter_pattern = self.chapter_patterns.get('ko')
        if chapter_pattern:
        ...
        # ====== 검색 전략 결정 ======
        article_count = len(result['article_filters'])

        if article_count == 1:
            # 단일 조항: exact_article (dense/sparse/keyword 모두 해당 조항만)
            result['search_strategy'] = 'exact_article'
        elif article_count >= 2:
            # 복수 조항: multi_article (hybrid 검색 + 각 조항 부스팅)
            result['search_strategy'] = 'multi_article'
        elif result['concept_keywords']:
            result['search_strategy'] = 'concept'
        else:
            result['search_strategy'] = 'hybrid'

        # 쿼리 최적화: 조항 번호 표현을 정규화하여 검색 노이즈 감소
        optimized = query
        if result['article_filters']:
            for article_num in result['article_filters']:
                optimized = re.sub(
                    rf'제\s*{re.escape(article_num)}\s*조|Article\s+{re.escape(article_num)}',
                    f'제{article_num}조',  # 공백 제거하여 정규화
                    optimized,
                    flags=re.IGNORECASE
                ).strip()
    ...
    def apply_constitution_boost(
        ...
        exact_article/multi_article 전략 시 해당 조항에 강력한 부스팅 적용.
        comparative_constitution_router.py에서 hybrid_search 결과에 후처리로 적용 가능.
        """
        ...
        for cand in candidates:
            ...
            if article_filters:
                ...
                if article_num in article_filters:
                    if search_strategy == 'exact_article':
                        boost += 0.8  # 단일 exact 모드: 강력 부스팅
                    else:
                        boost += 0.5  # multi_article 모드: 중간 부스팅
